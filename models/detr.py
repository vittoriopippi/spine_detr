...
class DETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
        ...
        # Passing output_dim from 4 to 2 because there are only the centers
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # Since is a two class problem we simplify the operation and set
        # 0 represent no object
        # 1 represent that is a vertebra
        target_classes = torch.zeros(src_logits.shape[:2], dtype=torch.int64, device=src_logits.device)
        target_classes_o = torch.ones_like(target_classes[idx])
        target_classes[idx] = target_classes_o
    ...
    def loss_binary_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_bce = F.binary_cross_entropy(src_logits.sigmoid(), target, reduction='none')
        loss_bce = loss_bce.sum() / num_boxes
        losses = {'loss_bce': loss_bce}

        if log:
            # TODO this should probably be a separate loss, not hacked in this one here
            src = src_logits.sigmoid().round()
            res = (src - target) ** 2
    ...
    def loss_centers(self, outputs, targets, indices, num_boxes):
        ...
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]
        target_boxes = torch.cat([t[idx[1]] for t, idx in zip(targets, indices)])[:, 1:3]

        loss_centers = F.mse_loss(src_boxes, target_boxes, reduction='none')

        losses = {}
        losses['loss_centers'] = loss_centers.sum() / num_boxes

        return losses

    def loss_spine_l1(self, outputs, targets, indices, num_boxes):
        assert 'pred_boxes' in outputs
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs['pred_boxes'][idx]
        target_boxes = torch.cat([t[idx[1]] for t, idx in zip(targets, indices)])[:, 1:3]
        ...
        losses = {}
        losses['loss_spine_l1'] = loss_l1.sum() / num_boxes
...
def build(args):
    ...
    if args.dataset_file == "coco":
        ...
        num_classes = 250
    elif args.dataset_file == "2d_spine":
        num_classes = 1
    else:
        num_classes = 20
    ...
    weight_dict['loss_centers'] = args.giou_loss_coef
    weight_dict['loss_giou'] = args.giou_loss_coef
    weight_dict['loss_spine_l1'] = args.bbox_loss_coef
    weight_dict['loss_bce'] = weight_dict['loss_ce']
    if args.masks:
    ...
    losses = ['binary_labels', 'centers', 'spine_l1']
    if args.masks:
        losses += ["masks"]
